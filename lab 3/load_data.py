...
def character_weapon_list(file_name: str, weapon_type: float) -> list:
    """
    Returns a filtered list of characters based on their weapon

    >>> character_weapon_list("xxx.csv", 3)
    ...
    >>> character_luck_list("xxx.csv", 3)
    ...
    >>> character_weapon_list("xxx.csv", 3)
    ...
    """
...
def load_data(file_name: str, filter_tuple: tuple) -> list[dict]:
    """
    Loads character data based on specified attribute and value.

    >>> load_data("xxx.csv", "attribute")
    ...
    >>> load_data("xxx.csv", "weapon")
    ...
    >>> load_data("xxx.csv", "gun")
    ...
    """
    valid_keys = ['Occupation', 'Strength', 'Luck', 'Weapon', 'All']
    ...
    if filter_key == 'Occupation':
        player_list = character_occupation_list(file_name, filter_value)
    elif filter_key == 'Strength':
        player_list = character_strength_list(file_name, filter_value)
    elif filter_key == 'Weapon':
        player_list = character_weapon_list(file_name, filter_value)
    elif filter_key == 'Luck':
        player_list = character_luck_list(file_name, filter_value)
    elif filter_key == 'All':
        with open(file_name, 'r') as file:
            ...
            for line in file:
                if first_line:
                    ...
                    attributes[6] = float(attributes[6])
                    character = {}
                    for i in range(len(attributes)):
                        if i == 1 or i == 2 or i == 3 or i == 4 or i == 5 or i == 7:
                            character[header[i]] = int(attributes[i])
                        elif i == 6:
                            character[header[i]] = float(attributes[i])
                        else:
                            character[header[i]] = attributes[i]
...
def calculate_health(characters: list) -> list:
    ...
    >>> calculate_health(3,1,8,"something"])
    ...
    >>> calculate_health(9,3,4,"something"])
    ...
    >>> calculate_health([3,1,4,"something"])
    ...    """
